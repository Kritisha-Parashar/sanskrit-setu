          ...
          <div className="flex items-center gap-2">
            <Link to="/profile">
              <Button
                variant="ghost"
                size="icon"
                className="text-primary-foreground hover:bg-primary/30 rounded-full"
              >
...
const Dashboard = () => {
  ...
  const displayName =
    isLoggedIn && user ? user.username || user.name || "Learner" : "Learner";
  ...
  const performanceCards = [
    ...
    },
    {
      id: "level",
      label: "Level",
      value: Math.floor(progress.xp / 500) + 1,
      icon: Crown,
      color: "text-accent",
    },
  ];
  ...
  const handleNext = () =>
    setCurrentSlide((prev) => (prev + 1) % (performanceCards.length / 2));
  const handlePrev = () =>
    setCurrentSlide((prev) =>
      prev === 0 ? performanceCards.length / 2 - 1 : prev - 1,
    );
        ...
        <div className="grid grid-cols-12 gap-10">
          <aside className="col-span-12 lg:col-span-3 sticky top-24 self-start h-fit space-y-6">
            <div className="bg-white/90 backdrop-blur-md rounded-[2.5rem] p-8 border border-white/50 shadow-xl group/hub relative overflow-hidden">
              <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-8 text-center opacity-60">
                Performance Hub
              </h4>
              ...
              <div className="relative mb-8 px-2">
                <button
                  onClick={handlePrev}
                  className="absolute -left-4 top-1/2 -translate-y-1/2 z-10 p-1.5 rounded-full bg-white shadow-md border border-black/5 opacity-0 group-hover/hub:opacity-100 transition-opacity hover:bg-primary hover:text-white"
                >
                  <ChevronRight className="w-4 h-4 rotate-180" />
                </button>
                <button
                  onClick={handleNext}
                  className="absolute -right-4 top-1/2 -translate-y-1/2 z-10 p-1.5 rounded-full bg-white shadow-md border border-black/5 opacity-0 group-hover/hub:opacity-100 transition-opacity hover:bg-primary hover:text-white"
                >
                      ...
                      >
                        {performanceCards
                          ...
                          .map((card) => (
                            <div
                              key={card.id}
                              className="bg-primary/5 p-4 rounded-2xl text-center border border-primary/10 transition-transform hover:scale-105"
                            >
                              <card.icon
                                className={`w-5 h-5 ${card.color} mx-auto mb-1 fill-current`}
                              />
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-end">
                    ...
                    <p className="text-xs font-black text-primary">
                      {Math.round((progress.completedLessons.length / 5) * 100)}
                      %
                    </p>
                  </div>
                  <Progress
                    value={(progress.completedLessons.length / 5) * 100}
                ...
                <Link to="/lectures" className="block group">
                  <div className="bg-primary hover:bg-primary-dark p-5 rounded-2xl text-white transition-all shadow-lg active:scale-95 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-xs font-black uppercase tracking-widest opacity-70 leading-none mb-1">
                        Explore Library
                      </p>
          ...
          <section className="col-span-12 lg:col-span-6 space-y-8 pb-20">
            ...
            <div className="bg-white/40 backdrop-blur-sm rounded-[3rem] p-12 border border-white/40 shadow-inner">
              <div className="flex items-center justify-center gap-4 mb-16">
                <h3 className="text-xs font-black text-muted-foreground uppercase tracking-[0.4em]">
                  Syllabus Path
                </h3>
              </div>
              ...
              <div className="relative flex flex-col items-center gap-16">
                ...
                {lessons.map((lesson) => {
                  const isUnlocked = progress.unlockedLessons.includes(
                    lesson.lessonId,
                  );
                  ...
                  return (
                    <Link
                      ...
                      className={`relative group ${!isUnlocked ? "opacity-30" : ""}`}
                    >
                      <div
                        className={`
                      ...
                      <div
                        className={`absolute left-full ml-10 top-1/2 -translate-y-1/2 bg-white/95 backdrop-blur-md p-6 rounded-2xl border border-black/[0.04] shadow-2xl opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 min-w-[220px]
                      ...
                      >
                        ...
                        <p className="text-[10px] font-black uppercase text-muted-foreground/50 tracking-[0.2em]">
                          {lesson.subtitle}
                        </p>
                      </div>
                    </Link>
          ...
          <aside className="col-span-12 lg:col-span-3 sticky top-24 self-start h-fit space-y-6">
            <div className="relative group">
              ...
              <Link
                to="/ai-scholar"
                className="relative block bg-white/95 backdrop-blur-md rounded-[2.5rem] p-8 border border-purple/20 shadow-xl overflow-hidden transition-transform hover:-translate-y-1"
              >
                ...
                <h4 className="font-display text-2xl font-bold text-foreground mb-2 leading-none">
                  AI Scholar
                </h4>
                <p className="text-sm text-muted-foreground leading-relaxed mb-6 font-medium">
                  Analyze any Sanskrit shloka with deep grammar insight
                  instantly.
                </p>
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-[2rem] p-6 border border-white/50 shadow-lg space-y-4">
              ...
              <Link
                to="/gurukul-lesson"
                className="flex items-center gap-4 p-4 rounded-2xl hover:bg-muted/50 transition-all group border-t border-black/5"
              >
